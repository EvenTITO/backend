...
class EventReviewerService(BaseService):
    ...
    async def get_all_reviewers(self, event_id: UUID):
    ...
    async def get_reviewer(self, event_id: UUID, user_id: UID):
    ...
    async def remove_reviewer(self, event_id: UUID, user_id: UID) -> None:
        if not await self.reviewer_repository.is_member(event_id, user_id):
            raise UserNotIsReviewer(event_id, user_id)
        await self.reviewer_repository.remove_member(event_id, user_id)

    async def is_reviewer(self, event_id: UUID, user_id: UID) -> None:
