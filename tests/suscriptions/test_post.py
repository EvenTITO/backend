...
async def test_post_suscription(client, user_data, event_data):
...
async def test_post_suscription_without_event_fails(
        client,
        user_data
):
...
async def test_post_suscription_without_user_fails(client, event_data):
    id_event = event_data['id']
